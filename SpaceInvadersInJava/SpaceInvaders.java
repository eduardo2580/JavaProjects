...
public class SpaceInvaders extends JPanel implements ActionListener, KeyListener {
    ...
    class Block {
        ...
        int hp = 1; //health points (default 1)
    ...
    int alienBulletVelocityY = 5; //alien bullet moving speed (downward)
    ...
    public void draw(Graphics g) {
        ...
        g.drawString("High Score: " + String.valueOf(highScore), 10, 70);
    }
    ...
    public void move() {
        ...
        for (int i = 0; i < alienArray.size(); i++) {
            ...
            if (alien.alive) {
                ...
                if (random.nextInt(300) < 2) { // ~2/300 chance per frame
        ...
        for (int i = 0; i < bulletArray.size(); i++) {
            ...
            for (int j = 0; j < alienArray.size(); j++) {
                ...
                if (!bullet.used && alien.alive && detectCollision(bullet, alien)) {
                    bullet.used = true;
                    alien.hp--; // Reduce alien HP
                    if (alien.hp <= 0) {
                        alien.alive = false;
                        alienCount--;
                        score += 100;
                    } else {
                        score += 25; // Points for damaging an alien
                    }
                }
            }
        }
    ...
    public void createAliens() {
        ...
        for (int c = 0; c < alienColumns; c++) {
            for (int r = 0; r < alienRows; r++) {
                ...
                // Assign HP based on alien type (image index)
                // Yellow aliens are toughest (3 HP), Magenta (2 HP), others (1 HP)
                if (randomImgIndex == 3) { // yellow
                    alien.hp = 3;
                } else if (randomImgIndex == 2) { // magenta
                    alien.hp = 2;
                } else {
                    alien.hp = 1; // default and cyan
                }
